...
export interface LedgerEntry {
  ...
  touches: ReadonlyMap<string, readonly string[]>; // area → slugs
...
export function register(root: string, file: string, touches: Record<string, string[]>): SrcId { throw new Error("mock-out"); }
/** intake routing: tag + one idempotent-by-hash entry; no copies, no sidecars */
export function route(root: string, file: string, toAreas: string[]): SrcId { throw new Error("mock-out"); }
...
export function credit(root: string, area: string, filled: string[], updated?: string[]): number { throw new Error("mock-out"); }
...
/** the librarian's debt list: what each source is still owed, per area */
export function outstanding(root: string, area: string): Map<SrcId, string[]> { throw new Error("mock-out"); }
