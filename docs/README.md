...
| M13 | Engagement-level `_client/` config resolution | **BUILT.** small; unblocks sharing client context across areas (and M14's profile) |
| M14 | Document profile — which sections/callouts/derived views exist, changeable mid-engagement; plus `body_omit` (authored + aggregated but not in the procedure body) and an `appendix-controls` register | **BUILT.** depends on M13 for where the profile lives; `body_omit` is what lets controls and pain points live in appendices instead of interrupting the steps |
...
| M16 | **Section model — one home per fact** (7 sections, tag-on-change, callout `note`/`detail`, step `Condition:`) | **BUILT — moves 3 (callout `note`/`detail`), 4 (step `Condition:`), 2 (tag on change, contract-only) and move 1's REGISTRY half.** The seven sections (Scope / At a Glance / Before You Start / Procedure / Outputs & Evidence / Key Controls / Known Issues) now live in `doc_model.SECTION_TITLES`, with the pre-M16 titles + slugs as aliases and `prerequisites`+`inputs` merged into `before-you-start`; mandatory set is scope/card/procedure. **Move 1's CONTENT WAVE is pending** — a drafter pass per fragment ("Content wave: 8 → 7 sections" in the drafter contract); until it runs, an 8-section fragment parses, renders and reconciles, with a WARNING naming it. A repetition fix, not a length fix; four independently shippable moves, cheapest first |
| M17 | **Stage gates** — draft-ready boundary (guard 8.5) + sticky `hold:` list | **BUILT — gate + sticky holds** (consolidate visibility waits on M12). the ladder never stops between `fill` and `review`, so the "am I happy with the verbs and nouns?" decision lands after the spend. Gate and sticky holds both live; consolidate visibility still waits on M12 |
