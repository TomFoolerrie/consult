...
| M16 | **Section model — one home per fact** (7 sections, tag-on-change, callout `note`/`detail`, step `Condition:`) | **BUILT — moves 3 (callout `note`/`detail`), 4 (step `Condition:`) and 2 (tag on change, contract-only).** Move 1 (7 sections) pending the section-slug decision — A–H stands; its migration is dispatched by M14's drift guard. A repetition fix, not a length fix; four independently shippable moves, cheapest first |
