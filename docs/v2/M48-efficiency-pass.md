...
| v1-only agents (consult-taxonomy, consult-placement) | `consult-taxonomy`: **KEEP behind the compatibility gate** — still dispatched on v1/legacy areas and named throughout `scripts/`. `consult-placement`: **RETIRED — deleted on the human's ruling, 2026-08-17** (its dispatch died at M45, nothing referenced or tested the file; A1 item 2 flagged it, the human said delete). v1 tests are law and none grepped it. |
