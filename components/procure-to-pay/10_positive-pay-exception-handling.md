...
This procedure covers the review and disposition of positive pay exception items — checks presented to the bank that do not match the positive pay issue file transmitted from NetSuite at check print under [[weekly-payment-run]]. It covers the investigation of each flagged item, the pay/return decision, and the default handling of undecided items. Transmission of the issue file itself is part of [[weekly-payment-run]] and is not repeated here.
...
| Systems | Chase Connect (exception presentation and disposition); NetSuite (check issue records) |
| Key inputs | Positive pay exception items in Chase Connect; positive pay issue file / check issue records in NetSuite |
| Key outputs | Pay / return decision on each exception item recorded in Chase Connect; default return of any undecided item |

### Before You Start

- **Positive pay issue file / check issue records in NetSuite** — [[weekly-payment-run]]; transmitted to Chase from NetSuite at check print on each date checks are issued, this is the record of what the Company actually issued (SRC-001, SRC-003; §7.6 of the prior SOP, SRC-006).
...
- **Chase Connect access** — the Accounts Payable Manager holds access to the positive pay exception queue (SRC-001).
