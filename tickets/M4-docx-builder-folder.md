...
- **Cover:** the single `#` title drives the cover; `subtitle` from the manifest
  is the cover subtitle (don't lose it — review #6). `--no-cover` leaves the
  Document Profile inline.
