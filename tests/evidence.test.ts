...
// A22 meets the write port: a published work product carries a card
import { sidecarCard } from "./helpers.ts";
test("publishSynthesis refuses an artifact without a card (sidecar or head) by name; with a sidecar it publishes", () => {
  const root = bareEngagement();
  const src = ledger.route(root, stage(root, "p.csv", "a,b\n1,2\n"), ["ap"]);
  const v = ledger.verifySource(root, src);
  mkdirSync(join(root, "_synthesis/x/run"), { recursive: true });
  const out = join(root, "_synthesis/x/run/dups-001.csv"); writeFileSync(out, "a,b\n1,2\n1,2\n");
  assert.throws(() => ledger.publishSynthesis(root, out, ["ap"], [{ id: v.id, hash: v.hash }]), (e: Error) => e.message.includes("card") && e.message.includes("dups-001.csv"));
  sidecarCard(root, out, { title: "dups-001", kind: "system-export", summary: "duplicate keys", keyItems: [] });
  assert.equal(ledger.publishSynthesis(root, out, ["ap"], [{ id: v.id, hash: v.hash }]).provenance, "synthesis");
});
